...
def main():
    ...
    if args.verbose is not None:
        logger = loggingext.configure_default({1: logging.INFO, 2: logging.DEBUG}[min(args.verbose, 2)],
                                                   with_publisher=True)
        if args.verbose < 3:
            loggingext.configure_default(logging.ERROR, name='jb.plugin.call', with_publisher=True)
    elif args.quiet is not None:
        logger = loggingext.configure_default({1: logging.ERROR, 2: logging.CRITICAL}[min(args.quiet, 2)],
                                                   with_publisher=True)
    else:
        logger = loggingext.configure_from_file(args.logger)
